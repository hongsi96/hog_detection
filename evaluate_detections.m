function [gt_ids, gt_bboxes, gt_isclaimed, tp, fp, duplicate_detections,cur_gt_ids] = ...
...
for d=1:nd
    ...
    %cur_gt_ids = strcmp(image_ids(d), gt_ids(1)); %will this be slow?
   
    cur_gt_ids=strcmp(image_ids(d),gt_ids);
    %fprintf(image_ids);
    %fprintf(image_ids(d));
    bb = bboxes(d,:);
    ovmax=-inf;

    for j = find(cur_gt_ids')%j = 1:npos%find(cur_gt_ids')
        %fprintf(j);
        bbgt=gt_bboxes(j,:);
